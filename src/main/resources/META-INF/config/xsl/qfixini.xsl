...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema"
    xmlns:qfix="http://baxter-it.com/qfix/conf" xmlns:c="http://baxter-it.com/config/component"
    ...
    <xsl:template match="qfix:session">
        ...
        <xsl:call-template name="CRLF" />
        <xsl:apply-templates select="qfix:option" />
        <xsl:if test="@beginString">
        ...
        <xsl:choose>
            <xsl:when test="@persistMessages = 'true' ">
                <xsl:text>PersistMessages=Y</xsl:text>
                <xsl:call-template name="CRLF" />
            </xsl:when>
            <xsl:when test="@persistMessages = 'false' ">
                <xsl:text>PersistMessages=N</xsl:text>
                <xsl:call-template name="CRLF" />
            </xsl:when>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template match="qfix:connection[@type='acceptor']">
        <xsl:text>SocketAcceptAddress=</xsl:text>
        <xsl:value-of select="@host" />
        <xsl:call-template name="CRLF" />
        <xsl:text>SocketAcceptPort=</xsl:text>
        <xsl:value-of select="@port" />
        <xsl:call-template name="CRLF" />
        <xsl:call-template name="connection-common" />
    </xsl:template>
    ...
    <xsl:template name="connection-common">
        <xsl:if test="@type">
        ...
        <xsl:if test="@reconnectInterval">
            <xsl:text>ReconnectInterval=</xsl:text>
            <xsl:value-of select="@reconnectInterval" />
            <xsl:call-template name="CRLF" />
        </xsl:if>
    </xsl:template>
